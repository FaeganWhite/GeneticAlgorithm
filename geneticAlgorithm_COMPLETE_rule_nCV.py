...
class individual:
    ...
    def test(self, data):
        ...
        for rule in self.rules:
            ...
            if result != -1:
                #
                if self.rules.index(rule) != 0 and self.rules.index(rule) != -1:
                    rule_position = self.rules.index(rule)
                    self.rules[rule_position], self.rules[rule_position-1] = self.rules[rule_position-1], self.rules[rule_position]
                # Return the result
                #
                rule.score += 1
                return result
        ...
        return result
    #
    def updateGeneOrder(self):
        #
        new_gene = []
        #
        for rule in self.rules:
            #
            new_gene += rule.getValues()
        #
        #print(f"gene from {len(self.gene)/rule_length} to {len(new_gene)/rule_length}")
        self.gene = new_gene
...
class rule:
    ...
    def getValues(self):
        #
        return_list = []
        #print(len(self.rule_values))
        #
        for a in self.rule_values:
            #
            return_list.append(a)
        #
        return_list.append(self.output)
        #
        return return_list
...
def produceOffspring(population, tournament_number, mutation_chance, mutation_size, min_val, max_val, training_data, rule_number, test_data):
    # Create an offspring array
    offspring = []
    ...
    updatePopulationFitness(offspring, test_data, 1)
...
def trainAlgorithm(population, iterations, tournament_number, mutation_chance, mutation_size, min_val, max_val, training_data, init_rule_number, cross_training_split_number):
    ...
    for a in range(iterations):
        ...
        for segment_number in range(len(data)):
            ...
            check_population = produceOffspring(population, tournament_number, mutation_chance, mutation_size, min_val, max_val, training_data, rule_number, training_data)
            #
            check_best_fitness = findBestIndividual(check_population).fitness
            #
            #print(f"rule number = {rule_number}, fitness = {check_best_fitness}")
            #
            ...
            #
                ...
                keep_population = check_population
        #
        print(f"best fitness: {findBestIndividual(keep_population).fitness}, ultimate fitness: {ultimate_fitness}")
        #
        ...
        #
            ...
            population = produceOffspring(copy.deepcopy(population), tournament_number, mutation_chance, mutation_size, min_val, max_val, training_data, ultimate_rule_number, training_data)
...
cross_training_split_number = 20
