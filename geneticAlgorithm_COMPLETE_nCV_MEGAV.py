...
def produceOffspring(population, tournament_number, mutation_chance, mutation_size, min_val, max_val, training_data, rule_mutation_chance, min_rule_number, max_rule_number, test_data, cross_training_split_number):
    # Create an offspring array
    offspring = []
    ...
    #
    offspring = replaceWorst(offspring, findBestIndividual(population))
    # return the offspring array
    return offspring 

#
def replaceWorst(population, copy_individual, message= False):
    # Pick a random individual to be the worst
    worst = population[round(random.random()*len(population))-1]
    ...
    #
    if message == True:
        print(f"Replacing {population.index(worst)} with fitness {worst.fitness} with big strong {copy_individual.fitness} fitness")
    # Replace the worst offspring with a deep copy best individual from the previous generation
    population[population.index(worst)] = copy.deepcopy(copy_individual)
    #
    return population
...
def trainAlgorithm(population, iterations, tournament_number, mutation_chance, mutation_size, min_val, max_val, training_data, rule_mutation_chance, min_rule_number, max_rule_number, cross_training_split_number):
    # Get the rule length (number of genes to make a rule) from the data
    rule_length = (len(training_data[0])*2)-1
    ...
    for a in range(iterations):
        ...
        for segment_number in range(len(data)):
            ...
            check_population = produceOffspring(population, tournament_number, mutation_chance, mutation_size, min_val, max_val, nCV_training_data, rule_mutation_chance, min_rule_number, max_rule_number, nCV_test_data, cross_training_split_number)
            #
            best_individuals.append(findBestIndividual(check_population))
            #
            GET THE 2 BEST HALVES AND PUT THEM TOGETHER?
            #
        ...
        for best_individual in best_individuals:
            #
            population = replaceWorst(population, best_individual, True)
...
def getNoisyData(data_set, multiplier, noise_size):
    #
    original_data = copy.deepcopy(data_set)
    #
    for a in range(multiplier):
        #
        data_set = data_set + original_data
        #
    random.shuffle(copy.deepcopy(data_set))
    #
    for data in range(len(data_set)):
        #
        for position in range(len(data_set[data])-1):
            #
            noise_value = random.randrange(-1, 1) * noise_size
            #if data == 0:
            #    print(data_set[data][position])
            #
            data_set[data][position] += noise_value
            #if data == 0:
            #    print(data_set[data][position])
            #print()
            #
            if data_set[data][position] < 0:
                #
                data_set[data][position] = 0
            #
            if data_set[data][position] > 1:
                #
                data_set[data][position] = 1
    #
    return data_set

# Evaluate the population fitness
def geneticSolve(population_size, rule_number, min_val, max_val, iterations, tournament_number, mutation_chance, mutation_size, data_location, fold_number, rule_mutation_chance, min_rule_number, max_rule_number, repeat_times, file_name, cross_training_split_number):
    ...
    if fold_number != 1 :
        ...
        for round in range(repeat_times):
            ...
            for segment_number in range(len(folded_data)):
                ...
                result = trainAlgorithm(population, iterations, tournament_number, mutation_chance, mutation_size, min_val, max_val, training_data, rule_mutation_chance, min_rule_number, max_rule_number, cross_training_split_number)
    ...
    # Otherwise, if there is only one segment (no k fold testing)
        ...
        for round in range(repeat_times):
            ...
            result = trainAlgorithm(population, iterations, tournament_number, mutation_chance, mutation_size, min_val, max_val, data, rule_mutation_chance, min_rule_number, max_rule_number, cross_training_split_number)
            # Print out information
...
mutation_amount = 0.9
rule_mutation_chance = 0.1
min_rule_number = 1
max_rule_number = 30
fold_number = 10
repeat_times = 10
data_location = "data/data1.txt"
cross_training_split_number = 2
...
geneticSolve(population_size, rule_number, min_val, max_val, generations, tournament_number, mutation_chance, mutation_amount, data_location, fold_number, rule_mutation_chance, min_rule_number, max_rule_number, repeat_times, file_name, cross_training_split_number)