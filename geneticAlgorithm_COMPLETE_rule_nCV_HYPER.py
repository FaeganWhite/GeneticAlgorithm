...
def findFitness(individual, testData):
    ...
    # For every rule
    for rule in individual.rules:
        # Add the rule score to the score list
        score.append(rule.score)
...
def updatePopulationFitness(population, testData, cross_training_split_number):
    # For every individual
    for individual in population:
        # Find its fitness
        individual.fitness = findFitness(individual, testData)*cross_training_split_number
...
def initiatePopulation(size, rule_number, min_val, max_val, data):
    ...
    updatePopulationFitness(population, data, 1)
...
def tournamentSelection(population, tournament_number):
    ...
    for parent in parents:
        ...
        elif parent.fitness == fittest_parent.fitness and len(parent.gene) < len(fittest_parent.gene):
            # Make the fittest parent the new parent
            fittest_parent = parent
...
def produceOffspring(population, tournament_number, mutation_chance, mutation_size, min_val, max_val, training_data, rule_number, test_data, cross_training_split_number):
    # Create an offspring array
    offspring = []
    ...
    for x in range(len(population)):
        ...
        # While the childs gene is too small
        while len(child.gene) < rule_number* rule_length:
            # Add a new random rule
            child.gene += generateRandomGene(rule_length, min_val, max_val)
        # Update the rules to get scores
        child.updateRules(training_data)
        # Get the fitness
        child.fitness = findFitness(child,test_data)
        # Create a list of rules to remove
        remove_rules = []
        # For every rule
        for rule in child.rules:
            # If the score is zero
            if rule.score == 0:
                # Add it to the list of rules to remove
                remove_rules.append(child.rules.index(rule))
        # Get the number of rules to remove
        scale_down = (len(child.gene) - rule_number*rule_length)/rule_length
        # While rules need to be removed
        while scale_down > 0:
            # If there are rules with 0 fitness
            if len(remove_rules) > 0:
                # Get the position of the rule
                remove_index = remove_rules[0]
                # Remove it form the list of rules to remove
                remove_rules = remove_rules[1:]
                # For every one of the rules value
                for a in range(rule_length):
                    # Replace it with an x
                    child.gene[remove_index*rule_length + a] = "x"
            # Decrement the counter
            scale_down -= 1
        # While there is x in the gene
        while "x" in child.gene: 
            # Remove x from the gene
            child.gene.remove("x")
        # Remove any extra rules from the end if not enoiugh have been removed
        child.gene = child.gene[:rule_number*rule_length]
        # Update the ruled#s
        child.updateRules(training_data)
        # Update the fitness
        child.fitness = findFitness(child,test_data)
    ...
    updatePopulationFitness(offspring, test_data, cross_training_split_number)
...
def trainAlgorithm(population, iterations, tournament_number, mutation_chance, mutation_size, min_val, max_val, training_data, init_rule_number, cross_training_split_number):
    # Get the rule length (number of genes to make a rule) from the data
    rule_length = (len(training_data[0])*2)-1
    ...
    ultimate_fitness = 0
    #
    ultimate_rule_number = init_rule_number
    # For every iteration
    for a in range(iterations):
        # Split the data
        data = kFoldSplit(training_data, cross_training_split_number)
        # Create a best fitness holder
        best_fitness = 0
        # Initiate an array to store the best value of the iteration
        keep_population = []
        # For every section of the data
        for segment_number in range(len(data)):
            # Get the rule number as an increment of the total rule number
            rule_number = init_rule_number - (round((init_rule_number / len(data))*segment_number)+1)
            # Get the training data
            nCV_training_data = getTrainingTestData(segment_number, data)[0]
            # Get the test data
            nCV_test_data = getTrainingTestData(segment_number, data)[1]
            # Evaluate the population and produce a new set of offspring
            check_population = produceOffspring(population, tournament_number, mutation_chance, mutation_size, min_val, max_val, nCV_training_data, rule_number, nCV_test_data, cross_training_split_number)
            # Get the best fitness of the new population
            check_best_fitness = findBestIndividual(check_population).fitness
            # If it's the best of the generation
            if check_best_fitness > best_fitness:
                # Store the fitness
                best_fitness = check_best_fitness
                # Replace the population
                keep_population = check_population
            # If it's equal but has less rules
            elif check_best_fitness >= best_fitness and len(findBestIndividual(check_population).gene) < len(findBestIndividual(keep_population).gene):
                # Store the fitness
                best_fitness = check_best_fitness
                # Replace the population
                keep_population = check_population
        # At the end of the generation, if the fitness is the best yet
        if findBestIndividual(keep_population).fitness > ultimate_fitness:
            # Use this population in the next generation
            population = keep_population
            # Store the fitness
            ultimate_fitness = findBestIndividual(keep_population).fitness
            # Store the rule number
            ultimate_rule_number = int(len(findBestIndividual(population).gene)/rule_length)
        # Otherwise if the fitness matches the best yet but it has fewer rules
        elif findBestIndividual(keep_population).fitness >= ultimate_fitness and len(findBestIndividual(keep_population).gene) < len(findBestIndividual(population).gene):
            # Use the population in the next generation
            population = keep_population
            # Store the fitness
            ultimate_fitness = findBestIndividual(keep_population).fitness
            # Store the rule number
        ...
        # ohterwise 
            ...
            # Use the current population plus 2 on the rule number for the next generation
            population = produceOffspring(copy.deepcopy(population), tournament_number, mutation_chance, mutation_size, min_val, max_val, nCV_training_data, ultimate_rule_number+2, nCV_test_data, cross_training_split_number)
        ...
        print(f"best fitness rule num: {len(findBestIndividual(population).gene)/rule_length}")
        print(f"ave rule num: {averageRuleNumber(population, rule_length)}")
        # Add new average rule number
        average_rule_numbers.append(len(findBestIndividual(population).gene)/rule_length)
...
# Get training and test data from folded data and a given fold
def getTrainingTestData(segment_number, data):
    # Copy the fold and use it as the test data
    test_data = copy.deepcopy(data[segment_number])
    # Copy the data and remove the test data to use as training data
    training_data = []
    # For every section of the data
    for training_segment_number, training_segment in enumerate(data):
...
def geneticSolve(population_size, rule_number, min_val, max_val, iterations, tournament_number, mutation_chance, mutation_size, data_location, fold_number, repeat_times, file_name, cross_training_split_number):
    ...
    if fold_number != 1 :
        ...
        for round in range(repeat_times):
            ...
            # For every section of the data
            for segment_number in range(len(folded_data)):
                # Get the training data
                #training_data = getNoisyData(getTrainingTestData(segment_number, folded_data)[0], 2, 0.2)
                training_data = getTrainingTestData(segment_number, folded_data)[0]
                # Get the test data
                test_data = getTrainingTestData(segment_number, folded_data)[1]
                # Print out information
                print("\nTraining poulation")
                ...
                result = trainAlgorithm(population, iterations, tournament_number, mutation_chance, mutation_size, min_val, max_val, training_data, rule_number, cross_training_split_number)
    ...
    # Otherwise, if there is only one segment (no k fold testing)
        ...
        for round in range(repeat_times):
            ...
            result = trainAlgorithm(population, iterations, tournament_number, mutation_chance, mutation_size, min_val, max_val, data, rule_number, cross_training_split_number)
            # Print out information
            ...
            output_array.append((f"{round},", f"{result[0].fitness/len(data)},", f"{math.floor(len(result[0].gene)/rule_length)},"))
...
cross_training_split_number = 10
...
geneticSolve(population_size, rule_number, min_val, max_val, generations, tournament_number, mutation_chance, mutation_amount, data_location, fold_number, repeat_times, file_name, cross_training_split_number)