...
def geneticSolve(population_size, rule_number, min_val, max_val, iterations, tournament_number, mutation_chance, mutation_size, data_location, fold_number, rule_mutation_chance, min_rule_number, max_rule_number):
...
# While the mutation chance
while rule_number <= 20:
    # While the mutation chance
    while mutation_amount <= 1:
        # While the mutation chance
        while mutation_chance <= 0.2:
            # Run the genetic algorithm
            result = geneticSolve(population_size, rule_number, min_val, max_val, generations, tournament_number, mutation_chance, mutation_amount, data_location, fold_number, rule_mutation_chance, min_rule_number, max_rule_number)
            with open('sweep.csv', 'a') as file:
                file.write(f"{rule_number}, {mutation_amount}, {mutation_chance}, {result}\n")
            # Close the file
            file.close()
            # Increment themutation chance
